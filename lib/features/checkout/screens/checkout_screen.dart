...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> initCall() async {
    ...
    Get.find<MarketLocationController>().getZone(
    ...
    widget.fromCart
        ? _cartList!.addAll(Get.find<MarketCartController>().cartList)
        : _cartList!.addAll(widget.cartList!);

    if (isLoggedIn) {
      if (Get.find<MarketProfileController>().userInfoModel == null &&
          Get.find<MarketProfileController>().userInfoModel?.userInfo == null) {
      ...
      if (Get.find<MarketAddressController>(tag: 'xmarket').addressList ==
        ...
        Get.find<MarketAddressController>(tag: 'xmarket')
            .getAddressList(canInsertAddress: true);
      }
    }
    ...
    Get.find<MarketCouponController>().setCoupon('', isUpdate: false);

    checkoutController.stopLoader(isUpdate: false);
    checkoutController.updateTimeSlot(0, false, notify: false);

    _isCashOnDeliveryActive = Get.find<MarketSplashController>(tag: 'xmarket')
        .configModel!
        .cashOnDelivery;
    _isDigitalPaymentActive = Get.find<MarketSplashController>(tag: 'xmarket')
        .configModel!
        .digitalPayment;
    _isOfflinePaymentActive = Get.find<MarketSplashController>(tag: 'xmarket')
            .configModel!
            .offlinePaymentStatus ??
        false;
    _isWalletActive = Get.find<MarketSplashController>(tag: 'xmarket')
            .configModel!
            .customerWalletStatus ??
        false;

    int digitalListCount = Get.find<MarketSplashController>(tag: 'xmarket')
            .configModel!
            .activePaymentMethodList
            ?.length ??
        0;
    debugPrint(
        'Payment Visibility - COD: $_isCashOnDeliveryActive, Digital: $_isDigitalPaymentActive, Offline: $_isOfflinePaymentActive, Wallet: $_isWalletActive, Digital List: $digitalListCount');
    ...
    if (AuthHelper.isLoggedIn()) {
      String phone = await _splitPhoneNumber(
          Get.find<MarketProfileController>().userInfoModel?.userInfo?.phone ??
              '');

      guestContactPersonNameController.text =
          '${Get.find<MarketProfileController>().userInfoModel?.userInfo?.fName ?? ''} ${Get.find<MarketProfileController>().userInfoModel?.userInfo?.lName ?? ''}';
      guestContactPersonNumberController.text = phone;
      guestEmailController.text =
          Get.find<MarketProfileController>().userInfoModel?.userInfo?.email ??
              '';
    }
  }
      ...
      body: guestCheckoutPermission || AuthHelper.isLoggedIn()
          ? GetBuilder<CheckoutController>(builder: (checkoutController) {
              return (checkoutController.distance != null &&
                  ...
                  ? GetBuilder<MarketLocationController>(
                      ...
                      return GetBuilder<MarketCouponController>(
                        ...
                        double addOnsPrice = _cartList != null
                            ? _calculateAddonsPrice(_cartList)
                            : 0;
                          ...
                          if (checkoutController.isFirstTime ||
                            ...
                            PlaceOrderBodyModel placeOrderBody =
                              ...
                              couponDiscountAmount:
                                  Get.find<MarketCouponController>().discount,
                              ...
                              paymentMethod: checkoutController
                                  ...
                                  : checkoutController.paymentMethodIndex == 1
                                      ...
                                      : checkoutController.paymentMethodIndex ==
                                          ...
                                          ? 'digital_payment'
                                          : 'offline_payment',
                              couponCode: (Get.find<MarketCouponController>()
                                              .discount! >
                                          0 ||
                                      (Get.find<MarketCouponController>()
                                                  .coupon !=
                              ...
                              cutlery:
                                  Get.find<MarketCartController>().addCutlery
                                      ? 1
                                      : 0,
                              ...
                              subscriptionStartAt:
                                  checkoutController.subscriptionOrder
                                      ...
                                      : '',
                              subscriptionEndAt: checkoutController
                                      .subscriptionOrder
                                  ? DateConverter.dateToDateAndTime(
                              ...
                              guestId: isGuestLogIn
                                  ? int.parse(Get.find<MarketAuthController>()
                                      .getGuestId())
                                  : 0,
                        ...
                        return Column(
                          children: [
                            WebScreenTitleWidget(
                                title: 'checkout_screen_title'.tr),
                            Expanded(
                                                ...
                                                Expanded(
                                                    ...
                                                    child: TopSectionWidget(
                                                      ...
                                                      guestAddressController:
                                                          guestAddressController,
                                                      guestStreetNumberController:
                                                          guestStreetNumberController,
                                                      guestHouseController:
                                                          guestHouseController,
                                                      guestFloorController:
                                                          guestFloorController,
                                                      guestNameNode:
                                                          guestNameNode,
                                                      guestEmailNode:
                                      ...
                                      : Column(
                                          ...
                                          children: [
                                              TopSectionWidget(
                                                ...
                                                guestStreetNumberNode:
                                                    guestStreetNumberNode,
                                                guestHouseNode: guestHouseNode,
                                                guestFloorNode: guestFloorNode,
                                              ),
                                              BottomSectionWidget(
                                                isCashOnDeliveryActive:
                                                    _isCashOnDeliveryActive!,
  ...
  double? _getDeliveryCharge(
    ...
    ConfigModel? configModel =
        Get.find<MarketSplashController>(tag: 'xmarket').configModel;
    ...
    if (address?.zoneData != null && address!.zoneData!.isNotEmpty) {
      try {
        ...
        zoneData =
            address.zoneData!.isNotEmpty ? address.zoneData!.first : null;
      }
    }
    ...
    double? maximumCharge = restaurant.selfDeliverySystem == 1
        ? restaurant.minimumShippingCharge
        : (zoneData?.maximumShippingCharge ?? 0);

    double deliveryCharge = checkoutController.distance! * perKmCharge;
    double charge = checkoutController.distance! * perKmCharge;

    debugPrint('--- Delivery Calculation ---');
    debugPrint('Distance: ${checkoutController.distance}');
    debugPrint('Per Km Charge: $perKmCharge');
    debugPrint('Minimum Charge: $minimumCharge');
    debugPrint('Self Delivery: ${restaurant!.selfDeliverySystem}');
    debugPrint('Initial Calculated Charge: $deliveryCharge');

    if (deliveryCharge < minimumCharge) {
      deliveryCharge = minimumCharge;
      charge = minimumCharge;
      debugPrint('Charge adjusted to minimum: $deliveryCharge');
    }
    ...
    if (deliveryCharge > maximumCharge!) {
    ...
    if (cartList != null) {
      for (var cartModel in cartList) {
        double multiplier = (cartModel.product!.isWeightBased ?? false)
            ? ((cartModel.requestedWeight != null &&
                    cartModel.requestedWeight! > 0)
                ? cartModel.requestedWeight!
                : (cartModel.quantity ?? 1).toDouble())
            : cartModel.quantity!.toDouble();

        price = price + (cartModel.product!.price! * multiplier);
  ...
  double _calculateExtraPackagingCharge(CheckoutController checkoutController) {
    if (((checkoutController.restaurant != null &&
                ...
                Get.find<MarketCartController>().needExtraPackage) ||
            (checkoutController.restaurant != null &&
                checkoutController.restaurant!.isExtraPackagingActive! &&
                checkoutController
                    .restaurant!.extraPackagingStatusIsMandatory!)) &&
  ...
  double _calculateReferralDiscount(double subTotal, double discount,
    ...
    if (Get.find<MarketProfileController>().userInfoModel != null &&
      ...
      if (Get.find<MarketProfileController>()
              ...
              .discountAmountType! ==
          "percentage") {
        referralDiscount = (Get.find<MarketProfileController>()
                    .userInfoModel!
                    .discountAmount! /
                100) *
        ...
        referralDiscount =
            Get.find<MarketProfileController>().userInfoModel!.discountAmount!;
      }
    }
