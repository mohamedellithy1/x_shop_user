...
class EstimatedArrivalTimeWidget extends StatelessWidget {
  ...
  const EstimatedArrivalTimeWidget(
      {super.key, required this.checkoutController});
  ...
  Widget build(BuildContext context) {
    ...
    if (isDineIn && checkoutController.selectedDineInDate != null) {
      String dayName =
          DateConverter.isToday(checkoutController.selectedDineInDate!)
              ? 'today'.tr
              : DateConverter.isTomorrow(checkoutController.selectedDineInDate!)
                  ? 'tomorrow'.tr
                  : 'custom'.tr;
      dineInDate =
          '$dayName (${DateConverter.containTAndZToUTCFormat(checkoutController.selectedDineInDate!.toString())})';
    }
              ...
              InkWell(
                onTap: () {
                  if (ResponsiveHelper.isDesktop(context)) {
                    Get.dialog(Dialog(
                        child: DineInDateSelectBottomSheet(
                ...
                child: Stack(clipBehavior: Clip.none, children: [
                  ...
                  Positioned(
                    top: -15,
                    left: 10,
                    child: checkoutController.selectedDineInDate != null
                        ? Container(
                            color: Theme.of(context).cardColor,
              ...
              InkWell(
                onTap: () async {
                  if (checkoutController.selectedDineInDate == null) {
                    ...
                    TimeOfDay time = _addHours(
                        TimeOfDay.now(),
                        checkoutController.restaurant!.dineInBookingDuration!,
                        checkoutController
                            .restaurant!.dineInBookingDurationTimeFormat!);
                    ...
                    if (pickedTime != null && pickedTime != TimeOfDay.now()) {
                      DateTime datetime =
                          DateConverter.formattingDineInDateTime(pickedTime,
                              checkoutController.selectedDineInDate!);

                      bool inTime = isInDineInSchedule(
                              checkoutController.timeSlots!,
                              datetime,
                              pickedTime,
                              checkoutController
                                  .restaurant!.dineInBookingDurationTimeFormat!,
                              checkoutController
                                  .restaurant!.dineInBookingDuration!)
                          .$1;
                      String? message = isInDineInSchedule(
                              checkoutController.timeSlots!,
                              datetime,
                              pickedTime,
                              checkoutController
                      ...
                      if (inTime) {
                        checkoutController.setEstimateDineInTime(
                            pickedTime.format(Get.context!));
                        checkoutController
                            .setOrderPlaceDineInDateTime(datetime);
                      } else {
                        showCustomSnackBar(message ??
                            'restaurant_is_close_on_your_selected_time'.tr);
                      }
                    } else {
                      if (kDebugMode) {
                ...
                child: Stack(clipBehavior: Clip.none, children: [
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(
                        child: Text(
                            checkoutController.estimateDineInTime ??
                                'select_time'.tr,
                            style: robotoRegular),
                      ),
                  ...
                  Positioned(
                    top: -15,
                    left: 10,
                    child: checkoutController.estimateDineInTime != null
                        ? Container(
                            color: Theme.of(context).cardColor,
                            padding: EdgeInsets.all(5),
  ...
  TimeOfDay _addHours(TimeOfDay time, int hoursOrMunitToAdd, String type,
    ...
    if (type == 'min') {
      final timeInMinutes =
          (time.hour * 60 + time.minute + hoursOrMunitToAdd + 1) % (24 * 60);
      final newHour = timeInMinutes ~/ 60;
      final newMinute = timeInMinutes % 60;

      return TimeOfDay(hour: newHour, minute: newMinute);
    } else if (type == 'hour') {
      final totalHours = (time.hour + hoursOrMunitToAdd) %
          24; // Wrap around 24 hours if necessary
  ...
  (bool, String?) isInDineInSchedule(
      List<TimeSlotModel>? timeSlots,
      DateTime datetime,
      TimeOfDay pickTime,
      String dineInTimeType,
      int duration) {
    TimeOfDay fixedTime = _addHours(TimeOfDay.now(), duration, dineInTimeType,
        fromSchedule: false);
    ...
    for (var v in timeSlots!) {
      if (datetime.isAfter(v.startTime!) && datetime.isBefore(v.endTime!)) {
        ...
        if ((dineInTimeType == 'hour' || dineInTimeType == 'min') &&
                DateConverter.isToday(datetime)
            ? !_isBeforeCurrentTime(pickTime, fixedTime)
            : true /* && !_isBeforeCurrentTime(pickTime, fixedTime)*/) {
          return (true, null);
          ...
          return (
            false,
            'dine_in_order_is_unavailable_at_your_selected_time'.tr
          );
        }
      }
    }
