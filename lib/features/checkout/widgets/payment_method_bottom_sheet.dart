...
class _PaymentMethodBottomSheetState extends State<PaymentMethodBottomSheet> {
  ...
  void configurePartialPayment() {
    if (!widget.isSubscriptionPackage &&
      ...
      debugPrint(
          'Partial Payment Config - walletBalance: $walletBalance, totalPrice: ${widget.totalPrice}, isPartialPay: ${Get.find<CheckoutController>().isPartialPay}');
      debugPrint(
          'Config States - PartialPayment: ${Get.find<MarketSplashController>(tag: 'xmarket').configModel!.partialPaymentStatus}, CustomerWallet: ${Get.find<MarketSplashController>(tag: 'xmarket').configModel!.customerWalletStatus}');
      ...
      if (Get.find<CheckoutController>().isPartialPay) {
        notHideWallet = false;
        String? partialMethod = Get.find<MarketSplashController>(tag: 'xmarket')
            .configModel!
            .partialPaymentMethod;
        debugPrint('Partial Method from Config: $partialMethod');
        if (partialMethod == 'cod') {
          ...
          notHideDigital = true;
          notHideOffline = false;
        } else if (partialMethod == 'offline_payment') {
          notHideCod = false;
          notHideDigital = false;
          notHideOffline = true;
        } else if (partialMethod == 'both') {
          notHideCod = true;
          notHideDigital = true;
          notHideOffline = true;
        }
      ...
      int digitalListLen = Get.find<MarketSplashController>(tag: 'xmarket')
              .configModel!
              .activePaymentMethodList
              ?.length ??
          0;
      int offlineListLen =
          Get.find<CheckoutController>().offlineMethodList?.length ?? 0;
      debugPrint(
          'Lists Lengths - Digital: $digitalListLen, Offline: $offlineListLen');
      notHideOffline = true;
      debugPrint(
          'Visibility results - notHideCod: $notHideCod, notHideDigital: $notHideDigital, notHideOffline: $notHideOffline, notHideWallet: $notHideWallet');
    }
  }
                  ...
                  child: Column(
                    children: [
                      ...
                      !widget.isSubscriptionPackage &&
                          ...
                          : const SizedBox(),
                      checkoutController.subscriptionOrder || !notHideCod
                          ? SizedBox()
        ...
        child: Container(
          decoration: image != null
              ...
              : BoxDecoration(
                  ...
                  border: Border.all(
                      color: isSelected && isDigitalPayment
                          ? Theme.of(context)
                              .primaryColor
  ...
  Widget changeAmountView(CheckoutController checkoutController) {
    return Column(
      children: [
        // checkoutController.showChangeAmount
        //     ? Container(
        //         decoration: BoxDecoration(
        ...
        //             children: [
        //               Text(
        //                   '${'change_amount'.tr} (EGP)',
        //                   style: robotoBold),
        //               Text('add_cash_amount_for_charge'.tr,
        ...
        //                 isEnabled: checkoutController.paymentMethodIndex == 0
        //                     ? true
        //                     : false,
        //                 onChanged: (String value) {
        //                   checkoutController
        //                       .setExchangeAmount(double.tryParse(value) ?? 0);
        //                 },
        //               ),
        //             ]),
        //       )
        ...
        //   onTap: () {
        //     checkoutController
        //         .setShowChangeAmount(!checkoutController.showChangeAmount);
        //   },
