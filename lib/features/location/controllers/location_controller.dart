...
class MarketLocationController extends GetxController implements GetxService {
  final LocationServiceInterface locationServiceInterface;

  MarketLocationController({required this.locationServiceInterface});
  ...
  Future<AddressModel> getCurrentLocation(bool fromAddress,
    ...
    try {
      ...
      if (kDebugMode) {
        print('❌ خطأ في فحص المبنى xmarket: $e');
      }
    }
  ...
  Future<ZoneResponseModel> getZone(String? lat, String? long, bool markerLoad,
    ...
    if (lat == '0' ||
        ...
        lat == null ||
        long == null) {
      return ZoneResponseModel(false, 'invalid_coordinates'.tr, [], []);
    }
    if (markerLoad) {
      _loading = true;
    } else {
  ...
  void autoNavigate(AddressModel? address, bool fromSignUp, String? route,
    ...
    if (route != 'home') {
      locationServiceInterface.handleRoute(fromSignUp, route, canRoute,
          restaurantId: restaurantId, isDesktop: isDesktop);
    }
  }
  ...
  Future<void> addAddressData() async {
    ...
    await getZone(
        _position.latitude.toString(), _position.longitude.toString(), false);
    update();
  }
