...
class _AccessLocationScreenState extends State<AccessLocationScreen> {
  ...
  void initState() {
    ...
    // إرسال الموقع الحالي تلقائياً عند فتح الشاشة
    if (!widget.fromHome) {
  ...
  Future<void> _getCurrentLocationAndRoute() async {
    ...
    if (response.isSuccess) {
      if (!Get.find<MarketAuthController>().isGuestLoggedIn() ||
        ...
        Get.find<MarketAuthController>().guestLogin().then((response) {
          if (response.isSuccess) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Builder(
        builder: (context) {
          final MarketAddressController addressControllerInstance =
              Get.find<MarketAddressController>(tag: 'xmarket');
          if (isLoggedIn) {
            addressControllerInstance.getAddressList();
          }
          return GetBuilder<MarketAddressController>(
            id: 'xmarket',
            global: false,
            init: addressControllerInstance,
            builder: (addressController) {
              return isLoggedIn
                  ? SingleChildScrollView(
                      physics: addressController.addressList != null &&
                        ...
                        child: SizedBox(
                            ...
                            child: Column(children: [
                              CustomAssetImageWidget(
                                  XmarketImages.deliveryLocation,
                                  height: 220),
                              const SizedBox(
