...
class OrderTrackingScreenState extends State<OrderTrackingScreen>
    ...
    if (Get.find<OrderController>().trackModel?.orderStatus != 'delivered' &&
      ...
      _timer = Timer.periodic(const Duration(seconds: 10), (timer) {
        if (Get.currentRoute.contains(RouteHelper.orderDetails) ||
          ...
          updateMarker(
            Get.find<OrderController>().trackModel?.restaurant,
            Get.find<OrderController>().trackModel?.deliveryMan,
            Get.find<OrderController>().trackModel?.orderType == 'take_away'
                ? Get.find<MarketLocationController>().position.latitude == 0
                    ? Get.find<OrderController>().trackModel?.deliveryAddress
                    : AddressModel(
                        latitude: Get.find<MarketLocationController>()
                            .position
                            .latitude
                            .toString(),
                        longitude: Get.find<MarketLocationController>()
                            .position
                            .longitude
                            .toString(),
                        address: Get.find<MarketLocationController>().address,
                      )
                : Get.find<OrderController>().trackModel?.deliveryAddress,
            Get.find<OrderController>().trackModel?.orderType == 'take_away',
          );
                      ...
                      background: Stack(children: [
                        GoogleMap(
                          ...
                          minMaxZoomPreference:
                              const MinMaxZoomPreference(0, 21),
                        ...
                        Positioned(
                          right: 10,
                          bottom:
                              ResponsiveHelper.isDesktop(context) ? 190 : 150,
                          child: InkWell(
  ...
  void setMarker(Restaurant? restaurant, DeliveryMan? deliveryMan,
    ...
    try {
      BitmapDescriptor restaurantImageData =
        ...
        width: 50,
        imagePath: XmarketImages.restaurantMarker,
      );
      BitmapDescriptor deliveryBoyImageData =
          await MarkerHelper.convertAssetToBitmapDescriptor(
        width: 50,
        imagePath: XmarketImages.deliveryManMarker,
      );
      BitmapDescriptor destinationImageData =
          await MarkerHelper.convertAssetToBitmapDescriptor(
        width: 50,
        imagePath: XmarketImages.myLocationMarker,
      );
      ...
      if (fromCurrentLocation && currentAddress != null) {
        ...
        _controller!.moveCamera(CameraUpdate.newCameraPosition(CameraPosition(
            target: currentLocation, zoom: GetPlatform.isWeb ? 7 : 18)));
      }

      if (!fromCurrentLocation) {
        _controller!.moveCamera(CameraUpdate.newCameraPosition(CameraPosition(
            target: centerBounds, zoom: GetPlatform.isWeb ? 7 : 18)));
  ...
  void updateMarker(Restaurant? restaurant, DeliveryMan? deliveryMan,
      AddressModel? addressModel, bool takeAway,
      {AddressModel? currentAddress, bool fromCurrentLocation = false}) async {
    try {
      BitmapDescriptor restaurantImageData =
