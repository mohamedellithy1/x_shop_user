...
class _ProductBottomSheetWidgetState extends State<ProductBottomSheetWidget> {
  ...
  final List<TextEditingController> _weightControllers = []; // kg
  final List<TextEditingController> _gramControllers = []; // grams
  ...
  void dispose() {
    ...
    for (var controller in _gramControllers) {
      controller.dispose();
    }
    super.dispose();
  }
  ...
  Future<void> _initCall() async {
    ...
    if (widget.cart != null && widget.cart!.requestedWeight != null) {
      _updateWeightControllers(Get.find<ProductController>().quantity!);
      double weight = widget.cart!.requestedWeight!;
      int kg = weight.truncate();
      int grams = ((weight - kg) * 1000).round();
      for (int i = 0; i < _weightControllers.length; i++) {
        _weightControllers[i].text = kg > 0 ? kg.toString() : '';
        if (i < _gramControllers.length) {
          _gramControllers[i].text = grams > 0 ? grams.toString() : '';
        }
      }
    } else {
      _updateWeightControllers(Get.find<ProductController>().quantity!);
  ...
  void _updateWeightControllers(int quantity) {
    // Sync kg controllers
    if (_weightControllers.length < quantity) {
      for (int i = _weightControllers.length; i < quantity; i++) {
        var controller = TextEditingController();
        controller.addListener(_calculateTotalWeights);
        _weightControllers.add(controller);
      }
    } else if (_weightControllers.length > quantity) {
      for (int i = _weightControllers.length - 1; i >= quantity; i--) {
        _weightControllers[i].dispose();
        _weightControllers.removeAt(i);
      }
    }
    // Sync gram controllers
    if (_gramControllers.length < quantity) {
      for (int i = _gramControllers.length; i < quantity; i++) {
        var controller = TextEditingController();
        controller.addListener(_calculateTotalWeights);
        _gramControllers.add(controller);
      }
    } else if (_gramControllers.length > quantity) {
      for (int i = _gramControllers.length - 1; i >= quantity; i--) {
        _gramControllers[i].dispose();
        _gramControllers.removeAt(i);
      }
    }
  }
  ...
  void _calculateTotalWeights() {
    double total = 0;
    for (int i = 0; i < _weightControllers.length; i++) {
      double kg = double.tryParse(_weightControllers[i].text) ?? 0;
      double grams = i < _gramControllers.length
          ? (double.tryParse(_gramControllers[i].text) ?? 0)
          : 0;
      total += kg + (grams / 1000.0);
    }
    setState(() {
      _totalWeights = total;
    });
            ...
            child: GetBuilder<ProductController>(
              builder: (productController) {
                ...
                double variationPriceWithDiscount =
                    _getVariationPriceWithDiscount(
                ...
                double multiplier = (product!.isWeightBased ?? false)
                    ? (_totalWeights > 0
                        ? _totalWeights
                ...
                double priceWithAddonsVariation =
                    ((price + variationPrice) * multiplier) + addonsCost;
                                    ...
                                    Padding(
                                      ...
                                      child: Column(
                                          ...
                                          children: [
                                                              ...
                                                              widget.isCampaign
                                                                  ...
                                                                  : () {
                                                                      if (!widget
                                                                          .isCampaign) {
                                                                        Get.toNamed(
                                                                            RouteHelper.getItemImagesRoute(product!));
                                                                      }
                                                                    },
                                                          child: Stack(
                                                              children: [
                                                                ...
                                                                : () {
                                                                    ...
                                                                    ResponsiveHelper.isMobile(
                                                                        ...
                                                                        ? showCustomBottomSheet(
                                                                            child:
                                                                                ProductReviewBottomSheet(product: product!),
                                                                            isDismissible: false,
                                                                            enableDrag: false)
                                                                        : Get.dialog(
                                                            ...
                                                            (Get.find<MarketSplashController>(
                                                                ...
                                                                ? Container(
                                                                    ...
                                                                    child: Row(
                                                                        children: [
                                                                          Image.asset(
                                                                              product!.veg == 1 ? XmarketImages.vegLogo : XmarketImages.nonVegLogo,
                                                                              height: 20,
                                                                              width: 20),
                                                                          const SizedBox(
                                                                          ...
                                                                          Text(
                                                                              ...
                                                                              style: robotoMedium.copyWith(fontSize: Dimensions.fontSizeDefault, color: marketThemeController.darkTheme ? Colors.white : Colors.black)),
                                                                        ]),
                                                                  )
                                                                : const SizedBox(),
                                                          ]),
                                            ...
                                            (product!.nutritionsName != null &&
                                                    product!.nutritionsName!
                                                        .isNotEmpty)
                                                ? Column(
                                                    crossAxisAlignment:
                                                        CrossAxisAlignment
                                            ...
                                            (product!.allergiesName != null &&
                                                    product!.allergiesName!
                                                        .isNotEmpty)
                                                ? Column(
                                                    crossAxisAlignment:
                                                        CrossAxisAlignment
                                                      ...
                                                      return Container(
                                                        ...
                                                        decoration: BoxDecoration(
                                                            color: product!.variations![index].required!
                                                                ? (product!.variations![index].multiSelect! ? product!.variations![index].min! : 1) <= selectedCount
                                                                  ...
                                                                  if (i == 4 &&
                                                                    ...
                                                                    return Padding(
                                                                      ...
                                                                      child:
                                                                          InkWell(
                                                                        onTap: () =>
                                                                            productController.showMoreSpecificSection(index),
                                                                        child: Row(
                                                                            children: [
                                                                        ...
                                                                        child: Row(
                                                                            children: [
                                                                              ...
                                                                              Text(
                                                                                ...
                                                                                overflow: TextOverflow.ellipsis,
                                                                                textDirection: TextDirection.ltr,
                                                                                style: productController.selectedVariations[index][i]! ? robotoMedium.copyWith(fontSize: Dimensions.fontSizeExtraSmall) : robotoRegular.copyWith(fontSize: Dimensions.fontSizeExtraSmall, color: Theme.of(context).disabledColor),
                                                                              )
                                                                            ]),
                                                                      ),
                                                                    );
                                                        ...
                                                        itemBuilder:
                                                          ...
                                                          double pieceWeight =
                                                              kg +
                                                                  (grams /
                                                                      1000.0);
                                                          double piecePrice = PriceConverter
                                                                  .convertWithDiscount(
                                                                      variationPrice +
                                                                          price,
                                                                      discount,
                                                                    ...
                                                                    child: Row(
                                                                        ...
                                                                        children: [
                                                                          Checkbox(
                                                                            ...
                                                                            side:
                                                                                BorderSide(width: 2, color: Theme.of(context).disabledColor),
                                                                          ),
                                                                          Text(
                                                                          ...
                                                                          Flexible(
                                                                            child:
                                                                              ...
                                                                              maxLines: 1,
                                                                              overflow: TextOverflow.ellipsis,
                                                                              style: product!.addOns![index].stockType != 'unlimited' && product!.addOns![index].addonStock != null && product!.addOns![index].addonStock! <= 0 ? robotoMedium.copyWith(color: Theme.of(context).colorScheme.error, fontSize: Dimensions.fontSizeExtraSmall) : robotoRegular.copyWith(color: Colors.transparent),
                                                                            ),
                                                                          ),
                                                                        ]),
                                                                  ),
                                                                              ...
                                                                              children: [
                                                                                Expanded(
                                                                                  child: InkWell(
                                                                                    ...
                                                                                    child: Center(
                                                                                        child: Icon(
                                                                                      (productController.addOnQtyList[index]! > 1) ? Icons.remove : CupertinoIcons.delete,
                                                                                      size: 18,
  ...
  void _onButtonPressed(
    ...
    if (productController.canAddToCartProduct) {
      CartModel cartModel = CartModel(
        ...
        priceWithAddonsVariation,
        priceWithDiscount,
        (priceWithAddonsVariation - priceWithDiscount),
        productController.quantity,
        addOnIdList,
        addOnsList,
        widget.isCampaign,
      ...
      if (widget.isCampaign) {
        ...
        Get.find<MarketCartController>().setNeedExtraPackage(false);
        Get.back();
  ...
  Future<void> _executeActions(
    ...
    if (product!.isWeightBased!) {
      if (widget.cart != null) {
        await cartController.removeCartItemOnline(widget.cart!.id!);
      } else if (productController.cartIndex != -1) {
        await cartController.removeCartItemOnline(
            cartController.cartList[productController.cartIndex].id!);
      }
      ...
      for (int i = 0; i < _weightControllers.length; i++) {
        ...
        double grams = i < _gramControllers.length
            ? (double.tryParse(_gramControllers[i].text) ?? 0)
            : 0;
        double? pieceWeight = kg + (grams / 1000.0);
        if (pieceWeight == 0) pieceWeight = null;
        if (pieceWeight != null && pieceWeight > 0) {
          OnlineCart pieceOnlineCart = OnlineCart(
            null,
            onlineCart.itemId,
            onlineCart.itemCampaignId,
            onlineCart.price!,
            onlineCart.variation,
            1,
            onlineCart.addOnIds!,
            onlineCart.addOns,
            onlineCart.addOnQtys!,
            onlineCart.model!,
            itemType: onlineCart.itemType,
            variationOptionIds: onlineCart.variationOptionIds,
            requestedWeight: pieceWeight,
          );
          await cartController.addToCartOnline(pieceOnlineCart,
              fromDirectlyAdd: true);
        }
      }
      Get.find<MarketCartController>().setNeedExtraPackage(false);
      Get.back();
