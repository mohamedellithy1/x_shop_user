...
import 'package:stackfood_multivendor/helper/market_middleware.dart';
...
class RouteHelper {
  ...
  static String getRestaurantRoute(int? id,
      {String? slug, bool fromDinIn = false}) {
...
/*  static String getCategoryProductRoute(int? id, String name) {
  ...
  static String getCuisineRestaurantRoute(int? cuisineId, String? name) {
    ...
    // Use cuisineId as fallback if slug is empty (e.g., for Arabic names or special characters)
    if (slug.isEmpty) {
      slug = cuisineId != null ? cuisineId.toString() : 'cuisine';
    }
          ...
          return getRoute(OrderSuccessfulScreen(
            ...
            contactPersonNumber: Get.parameters['contact_number'] != null &&
                ...
                : Get.find<MarketAuthController>().isGuestLoggedIn()
                    ? Get.find<MarketAuthController>().getGuestNumber()
                    : null,
...
// Best Reviewed Foods Route
  ...
  static Widget getRoute(Widget navigateTo, {bool byPuss = false}) {
    ...
    if (GetPlatform.isAndroid) {
      minimumVersion = splashController.configModel?.appMinimumVersionAndroid ?? 0;
    } else if (GetPlatform.isIOS) {
      minimumVersion = splashController.configModel?.appMinimumVersionIos ?? 0;
    }
    Widget targetScreen = AppConstants.appVersion < minimumVersion!
        ? const UpdateScreen(isUpdate: true)
        : MaintenanceHelper.isMaintenanceEnable()
            ? const UpdateScreen(isUpdate: false)
            : navigateTo;

    // Wrap with xMarket theme
    return _MarketThemeWrapper(child: targetScreen);
  }
}

// Wrapper widget to apply xMarket theme
class _MarketThemeWrapper extends StatefulWidget {
  final Widget child;

  const _MarketThemeWrapper({required this.child});

  @override
  State<_MarketThemeWrapper> createState() => _MarketThemeWrapperState();
}

class _MarketThemeWrapperState extends State<_MarketThemeWrapper> {
  @override
  Widget build(BuildContext context) {
    // Safely get ThemeController with try-catch to handle initialization issues
    try {
      final themeController = Get.find<MarketThemeController>(tag: 'xmarket');
      return Theme(
        data: themeController.darkTheme ? dark_theme.dark : light_theme.light,
        child: widget.child,
      );
    } catch (e) {
      // If ThemeController is not ready yet, use default light theme
      // This can happen during initial app startup
      return Theme(
        data: light_theme.light,
        child: widget.child,
      );
    }
  }
